...
import { RestaFracciones as Questions } from "@/lib/quizz/AritmeticaQuizzes";
...
function QuizzPage() {
  ...
  const handleFinish = () => {
    ...
    const quizId = currentTopic?.quizzesTopic.find((quiz) => quiz.path === pathName)?.id || '';

    const questions: { [key: string]: string } = response.reduce((acc, item: ResponseType) => {
      acc[`question${item.idQuestion}` as string] = item.idOption;
      return acc;
