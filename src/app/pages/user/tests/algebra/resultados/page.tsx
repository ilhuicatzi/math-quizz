import {
  ...
  async function loadLastQuizz() {
    try {
      ...
      const quizzAlgebra = await prisma.quizz.findMany({
        where: {
          titulo: 'Álgebra',
  ...
  async function AlgebraResultados() {
    const lastQuizz = await loadLastQuizz();
    const correctAnswers = AlgebraQuestions.map((question) => question.correctAnswer);
  ...
  export default AlgebraResultados